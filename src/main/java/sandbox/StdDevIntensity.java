...
import net.imagej.ops.statistics.StdDevRealTypeDirect;
...
public class StdDevIntensity<T extends RealType<T>>
		extends
	...
	protected RandomAccessibleInterval<DoubleType> safeCompute(
		...
		ops.run(Map.class, output, s.neighborhoods(interval),
				ops.op( StdDevRealTypeDirect.class, DoubleType.class, Iterable.class));
	...
	public static void main(String[] args) {
		...
		try {
			...
			Img<DoubleType> run = (Img<DoubleType>) ops.run( 
					StdDevIntensity.class, Img.class, image, 3 );
			
			long end = System.currentTimeMillis();
			
			System.out.println( "StdDevIntensity took " + (end-start) + " ms");
