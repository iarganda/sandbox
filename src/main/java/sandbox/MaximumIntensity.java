...
@Plugin( type = Op.class, name = "maximum" )
public class MaximumIntensity<T extends RealType<T>>
		extends
	...
	protected RandomAccessibleInterval<DoubleType> safeCompute(
		...
		ops.run(Map.class, output, s.neighborhoods(interval),
				ops.op( Max.class, DoubleType.class, Iterable.class));
	...
	public static void main(String[] args) {
		...
		try {
			...
			Img<DoubleType> run = (Img<DoubleType>) ops.run( 
					MaximumIntensity.class, Img.class, image, 1 );
			
			long end = System.currentTimeMillis();
			
			System.out.println( "MaximumIntensity took " + (end-start) + " ms");
