...
@Plugin( type = Op.class, name = "minimum" )
public class MinimumIntensity<T extends RealType<T>>
		extends
	...
	protected RandomAccessibleInterval<DoubleType> safeCompute(
		...
		ops.run(Map.class, output, s.neighborhoods(interval),
				ops.op( Min.class, DoubleType.class, Iterable.class));
		...
		try {
			Object image = 
				//io.open( "/Users/Nachete/data/challenge-2d/train-volume-1.tif" );
					io.open( "/Users/Nachete/data/flybrain-32bit.tif" );
			long start = System.currentTimeMillis();
			
			Img<DoubleType> run = (Img<DoubleType>) ops.run( 
					MinimumIntensity.class, Img.class, image, 4 );
			
			long end = System.currentTimeMillis();
			
			System.out.println( "MinimumIntensity took " + (end-start) + " ms");
